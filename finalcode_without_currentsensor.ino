...
void loop(){
  ...
  cellMax_index = -1;  //
   ...
   for (int i = 0; i < 12 ; i++) // finding cell with minimun and max charge
  {
 ...
 digitalWrite(dischargeRelayPin, dischargeRelay_state ); // turn off relays during any critical issue
 digitalWrite(chargeRelayPin, chargeRelay_state );    // turn off relays during any critical issue

  // passive balancing, balance all cells down to lowest cell voltage
  
  tx_cfg[0][4] = 0x00; // clears S1-8
  tx_cfg[0][5] = tx_cfg[0][5]  & 0xF0 ; // clears S9-12 and sets software timer to 0.5 min
  
 for(i = 0; i < 12;i++)
 {        
  if (cell_codes[0][i] * 0.0001 >= cellMin_V) 
  {
    if (i >= 0 && i <= 7) {
    tx_cfg[0][4] = tx_cfg[0][4] | 1 << i; // setting up DCC[i] values for s1 to s8
  }
    if ( i > 7) {
    tx_cfg[0][5] = tx_cfg[0][5] | ( 1 << (i - 8)); // setting up DCC[i] values for s9 to s12
  }
  }
  }
...
void init_cfg(){
  for(int i = 0; i<TOTAL_IC;i++){
    ...
    tx_cfg[i][5] = 0x10 ; // sets the software timer to 1 minute // check datasheet for setting this value
  }
}
...
// calculate battery pack temperature using sensors connected to the arduino
